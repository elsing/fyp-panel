    ...
    return(
        ...
        <form className="flex flex-row gap-2">
            <Select className="w-4/6" {...register("delta")}>
                {data?.success ? data?.json.map((delta: IDelta) => {
                    return(
                        <option value={delta.delta_id} key={delta.delta_id}>{delta.name}</option>
                    )
                }) : (
                    <option key="nodeltas" value={0}>No Deltas</option>
                )
                }
            </Select> 
            <Button className="w-2/6" onClick={handleSubmit(handleSelectDelta)}>Select</Button>
        </form>
        </div>
    )
}